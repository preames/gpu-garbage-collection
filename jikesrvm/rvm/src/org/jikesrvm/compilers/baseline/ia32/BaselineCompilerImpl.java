  ...
  protected final void emit_resolved_putfield(FieldReference fieldRef) {
    ...
    if (field.isReferenceType()) {
      ...
      if (NEEDS_OBJECT_PUTFIELD_BARRIER && !field.isUntraced()) {
          // NOTE: This code is unused in the current version.  Try the
          // GPU2 build for this option, but don't expect it to work!
        if (DO_COMPILED_REF_GRAPH_UPDATES) {
          // Compile in code to update the GPU reference graph too.
          // This is an epic violation of Jikes' modular design, but it avoids
          // having to scan the reference array on every putfield

          // java.lang.ref.Reference has its referenceOffsets changed at runtime
          // when -X:gc:noReferenceTypes marks its reference field traced
          // (see org.jikesrvm.mm.mminterface.MemoryManager.postBoot())
          // so don't try to optimize stores to that class or its subclasses.
          RVMClass fieldClass = field.getDeclaringClass();
          if (!DynamicTypeCheck.instanceOfClass(RVMType.JavaLangRefReferenceType, fieldClass.getTypeInformationBlock())) {
            int[] offsets = fieldClass.getReferenceOffsets();
            int refIndex;
            for (refIndex = 0; refIndex < offsets.length; refIndex++) {
              if (offsets[refIndex] == fieldOffset.toInt()) {
                // First update the field like normal
                ...
                // Now update reference graph
                asm.emitMOV_Reg_RegDisp(S0, S0, ObjectModel.GC_HEADER_OFFSET);
                asm.emitTEST_Reg_Reg(S0, S0);
                ForwardReference srcHasNoNode = asm.forwardJcc(Assembler.EQ);
                asm.emitTEST_Reg_Reg(T0, T0);
                ForwardReference isDstNull = asm.forwardJcc(Assembler.EQ);
                asm.emitMOV_Reg_RegDisp(T0, T0, ObjectModel.GC_HEADER_OFFSET);
                isDstNull.resolve(asm);
                asm.emitMOV_RegDisp_Reg(S0, Offset.fromIntZeroExtend((2 + refIndex) << LOG_BYTES_IN_ADDRESS), T0);
                srcHasNoNode.resolve(asm);
                return;
              }
            }
            System.out.println("GPU: could not find reference index of field " + field);
            for (refIndex = 0; refIndex < offsets.length; refIndex++)
              System.out.println("offsets[" + refIndex + "] = " + offsets[refIndex]);
            //VM.sysFail("GPU: could not find reference index of field " + field);
            // fall back to write barrier
          }
        }
